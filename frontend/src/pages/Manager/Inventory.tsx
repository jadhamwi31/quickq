...
export default function Inventory() {
	...
	useEffect(() => {
		...
		socket!.on(
			"update_order",
			(orderId, { dishesToMutate, dishesToRemove }) => {
				// new order
				console.log(orderId, { dishesToMutate, dishesToRemove });
			}
		);
	}, []);
	...
	const updateInventoryItems = async (index: number, itemName: string) => {
		const response = await fetch("/inventory/items/" + itemName, {
			method: "PUT",
			body: JSON.stringify({
				available: availableItems[index],
				needed: neededItems[index],
			}),
			credentials: "include",
			headers: {
								...
								<tr key={item.name}>
									...
									<td>
										<input
											...
											onKeyPress={(e) => {
												...
												if (
													charCode > 31 &&
													(charCode < 48 || charCode > 57) && // Check for digits
									...
									<td>
										<input
											...
											onChange={(e) =>
												handleNeededChange(index, e.target.value)
											}
											onKeyPress={(e) => {
												// Allow only decimal numbers and backspace
												const charCode = e.which ? e.which : e.keyCode;
